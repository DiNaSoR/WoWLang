...
-- Ported from Plumber (Docs/Plumber/Modules/ControlCenter/SettingsPanelNew.lua).
-- This is WoWLang / WoWAR's primary settings UI (Plumber-style ControlCenter panel).
-- NOTE: All templates are renamed to WOWTR_* to avoid collisions with Plumber if both addons are installed.
...
local API = ControlCenter.API
local LandingPageUtil = ControlCenter.LandingPageUtil

-- Minimal localization shim: use WOWTR.Config.Label when available, otherwise fall back to English keys.
local function _Label(key, fallback)
...
local L = setmetatable({}, {
    __index = function(_, k)
        return _Label("ControlCenter_" .. tostring(k):gsub("%s+", "_"), tostring(k))
    end,
})

local function GetDBBool(dbKey)
    return ControlCenter:GetDBBool(dbKey)
end

local function SetDBValue(dbKey, value, noApply)
    return ControlCenter:SetDBValue(dbKey, value, noApply)
end
...
local Def = {
    TextureFile = ControlCenter.Assets and ControlCenter.Assets.ControlCenterSprite or "";
    ButtonSize = 28,
    WidgetGap = 14,
    PageHeight = 576,
    CategoryGap = 40,
...
local MainFrame = CreateFrame("Frame", nil, UIParent, "WOWTR_SettingsPanelLayoutTemplate");
...
local function CreateNewFeatureMark(button, smallDot)
    ...
    local tex = (ControlCenter.Assets and ControlCenter.Assets.Path and ControlCenter.Assets.Path("Images\\ControlCenter\\NewFeatureTag.tga")) or nil
    newTag:SetTexture(tex, nil, nil, smallDot and "TRILINEAR" or "LINEAR");
...
do
    ...
    function CreateSearchBox(parent)
        local f = CreateFrame("EditBox", nil, parent, "WOWTR_EditBoxArtTemplate");
...
do
    function MenuInfo.InfoGetter_ModuleFilter()
        ...
        local widgets = {
            {type = "Header", text = _Label("ControlCenter_Sort_By", "Sort By")},
        };

        local selectedIndex = ControlCenter:UpdateCurrentSortMethod();
        local sortLabels = {
            _Label("ControlCenter_Sort_Alphabet", "Alphabetical"),
            _Label("ControlCenter_Sort_Date", "Date added"),
        }
        for i = 1, ControlCenter:GetNumFilters() do
            tinsert(widgets, {
                type = "Radio",
                text = sortLabels[i] or ("Sort " .. i),
    ...
    function MenuInfo.InfoGetter_ChangelogMenu()
        ...
        local p = WOWTR and WOWTR.db and WOWTR.db.profile
        local selectedIndex = (p and p.core and p.core.controlCenterChangelogFontSizeIndex) or 1;
        local sizes = {12, 14};
        ...
        for k, v in ipairs(sizes) do
            table.insert(widgets, {
                ...
                onClickFunc = function()
                    if p then
                        p.core = p.core or {}
                        p.core.controlCenterChangelogFontSizeIndex = k
                    end
...
do
    ...
    function CreateSquareButton(parent)
        local f = CreateFrame("Button", nil, parent, "WOWTR_SquareButtonArtTemplate");
...
do
    ...
    function OptionToggleMixin:OnClick(button)
        if self.onClickFunc then
            self.onClickFunc(self, button);
            -- WoWLang: per-module movable option dialogs are not implemented yet.
            -- Keep the ControlCenter panel open/maximized.
        end
    end
...
do
    ...
    function EntryButtonMixin:OnClick()
        if self.dbKey and self.data.toggleFunc then
            ...
            SetDBValue(self.dbKey, newState, true);
    ...
    function CreateSettingsEntry(parent)
        local f = CreateFrame("Button", nil, parent, "WOWTR_SettingsPanelEntryTemplate");
...
do
    ...
    function CreateSettingsHeader(parent)
        local f = CreateFrame("Frame", nil, parent, "WOWTR_SettingsPanelHeaderTemplate");
        Mixin(f, HeaderMixin);
...
do
    ...
    local outQuart = (ControlCenter.EasingFunctions and ControlCenter.EasingFunctions.outQuart) or function() end;
    ...
    function CreateSelectionHighlight(parent)
        local f = CreateFrame("Frame", nil, parent, "WOWTR_SettingsAnimSelectionTemplate");
...
do  --Search
    function MainFrame:RunSearch(text)
        if text and text ~= "" then
            ...
            -- IMPORTANT: GetSortedModules is a ':' method; wrap to preserve self binding.
            self.listGetter = function()
        ...
        -- Newly created pooled objects may appear after first Apply; ensure Arabic fonts are applied.
        ApplyArabicFonts(self);
    end
end
...
CreateUI = function()
    ...
    do
        ...
        FilterButton.onClickFunc = function(self)
            LandingPageUtil.DropdownMenu:ToggleMenu(self, MenuInfo.InfoGetter_ModuleFilter);
        end
        ...
        MainFrame.primaryCategoryPool = LandingPageUtil.CreateObjectPool(Category_Create, Category_Acquire);
    ...
    do  --RightSection
        ...
        mask:SetTexture(ControlCenter.Assets and ControlCenter.Assets.ControlCenterPreviewMask or nil, "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE");
    ...
    do  --CentralSection
        ...
        Background:SetTexture(ControlCenter.Assets and ControlCenter.Assets.ControlCenterBackground or nil);
        ...
        ScrollView:SetNoContentAlertText(_Label("ControlCenter_NoSearchResults", "No results"));
    ...
    local NineSlice = LandingPageUtil.CreateExpansionThemeFrame(MainFrame.FrameContainer, 10);
    MainFrame.NineSlice = NineSlice;
    -- WoWLang tweak: keep the outer border flush with the window edges.
    -- (Plumber insets this, but our current art/layout looks better without the inset.)
    NineSlice:CoverParent(0);
...
do  --ChangelogTab
    ...
    function Formatter:LoadUserFont()
        ...
        local p = WOWTR and WOWTR.db and WOWTR.db.profile
        local sizeIndex = (p and p.core and p.core.controlCenterChangelogFontSizeIndex) or 1;
        if sizeIndex == 2 then
    ...
    function InitChangelogTab()
        ...
        MenuButton.onClickFunc = function(self)
            LandingPageUtil.DropdownMenu:ToggleMenu(self, MenuInfo.InfoGetter_ChangelogMenu);
        end
        ...
        Tab2.VersionButtonPool = LandingPageUtil.CreateObjectPool(VerisonButton_Create);
    ...
    function MainFrame:ShowChangelog(versionID)
        ...
        if not self.redactorPool then
            local function Redactor_Create()
                local f = ControlCenter.CreateTextRedactor and ControlCenter.CreateTextRedactor(self.ChangelogTab.ScrollView) or CreateFrame("Frame", nil, self.ChangelogTab.ScrollView);
                f:SetColor(76/255, 69/255, 65/255);
                return f
            end
            self.redactorPool = LandingPageUtil.CreateObjectPool(Redactor_Create);
        end
                ...
                if info.dbKey then
                    ...
                    content[n] = {
                        ...
                        setupFunc = function(obj)
                            ...
                            if ControlCenter.Assets and ControlCenter.Assets.Path then
                                obj:SetTexture(ControlCenter.Assets.Path("Images\\ControlCenter\\Preview_"..tostring(info.dbKey)..".jpg"));
                            end
                        end;
                    };
                end
...
do  --Tab Buttons
    local TabInfo = {
        {name = function() return _Label("ControlCenter_Modules", "Modules") end, tabKey = "ModuleTab"},
        {name = function() return _Label("ControlCenter_Release_Notes", "Release Notes") end, tabKey = "ChangelogTab", initFunc = InitChangelogTab},
    };
    ...
    local function CreateTabButton(parent)
        local f = CreateFrame("Button", nil, parent, "WOWTR_TabButtonTemplate");
    ...
    function MainFrame:UpdateTabButtons()
        if not self.tabButtonPool then
            ...
            self.tabButtonPool = LandingPageUtil.CreateObjectPool(TabButton_Create);
...
do  --MainFrame Minimize UI while certain module options are shown
    function MainFrame:OnShow()
        -- WoWLang: module option dialogs are not implemented yet.
    end
    ...
    function MainFrame:InitMinimizedFrame()
        ...
        local NineSlice = ControlCenter.CreateNineSliceFrame(f);
        ...
        local ReturnButton = LandingPageUtil.CreateRedButton(f);
...
function MainFrame:ShowUI(mode)
    if CreateUI then
        ...
        if ControlCenter.InitializeModules then
